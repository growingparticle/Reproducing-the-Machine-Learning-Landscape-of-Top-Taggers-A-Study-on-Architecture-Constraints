...
The primary objective of this project is to extract the top quark signal from Quantum Chromodynamics (QCD) background noise and to compare different AI architectures.
...
- Simulation Framework: Monte Carlo simulated proton-proton collision events at a center-of-mass energy of s​=14 TeV, generated using Pythia8 and Delphes for ATLAS detector simulation.
...
* **`1_simple_condition_deep_sets.ipynb`**: Implements a baseline Deep Sets architecture utilizing *early fusion*. Global conditioning variables are concatenated at the individual particle level before processing, allowing the network to learn early context at the cost of memory.
...
* **`5_MC_deep_set.ipynb`**: Acts as the scientific control experiment. Restricts the architecture on the real Monte Carlo data to $k=0$ (Deep Sets), mathematically blinding the AI to local geometry. The resulting performance plateau confirms that resolving spatial details without millions of training examples requires graph edges.
...
If you have any questions or suggestions regarding the implementation or the numerical results of this study, feel free to reach out to Mehmet Ehliz.
